...
UTK = {
    ...
    alarmblip,
    text = { -- Texts
        loudstart = "[~r~E~w~] Basarak ~g~Sesli~w~ Soygunu Başlatabilrisiniz",
        silentstart = "[~r~E~w~] Basarak ~g~Sessiz~w~ Soygunu Başlatabilrisiniz",
        usecard = "[~r~E~w~] Basarak Personel Kartını Kulllanabilrisiniz",
        usethermal = "[~r~E~w~] Basarak Termal Aleti Kullanabilrisiniz",
        usehack = "[~r~E~w~] Hack Güvenlik Paneli",
        ...
        card = "Kart Kullanılıyor",
        thermal = "Termal Alet Yerleştiriliyor",
        burning = "Kilit Eritiliyor",
        lockpick = "Kapı Zorlanılıyor",
        using = "Panel İşleniyor",
        used = "İşleme Tamamlandı.",
        stage = "Tamamlandı.",
        search = "Aranıyor",
        hacking = "Hackleniyor",
        melted = "Kapı Kiliti Eritildi.",
        hacked = "Hack Tamamlandı.",
        unlocked = "Kapı Açıldı.",
        nothing = "Birşey Bulamadın.",
        found = "Personel Kartı Buldun.",
        time = "Kalan Süre: "
    }
}
UTK.searchinfo = {
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        ESX.TriggerServerCallback("utk_oh:checkItem", function(output)
                                            ...
                                            elseif not output then
                                                exports['mythic_notify']:SendAlert("error", "Flash Belleğe Sahip Değilsin.")
                                            end
                                        end, "weedburn")
                                    end
                                end
                                ...
                                if dst <= 4 then
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        ...
                                        if output then
                                            ...
                                            self:IdCard()
                                        elseif not output then
                                            exports["mythic_notify"]:SendAlert("error", "Personel Kartına Sahip Değilsin")
                                        end
                                        end, "id_card")
                                    end
                                end
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        ESX.TriggerServerCallback("utk_oh:checkItem", function(output)
                                            if output then
                                                ...
                                                self:Plant()
                                            elseif not output then
                                                exports['mythic_notify']:SendAlert("error", "Termal Alete Sahip Değilsin")
                                            end
                                        end, "thermal_charge")
                                    end
                                end
                                        ...
                                        if dst <= 1 and IsControlJustReleased(0, 38) then
                                            ESX.TriggerServerCallback("utk_oh:checkItem", function(output)
                                                if output then
                                                    ...
                                                    self:Lockpick()
                                                elseif not output then
                                                    exports['mythic_notify']:SendAlert("error", "Maymuncuğa Sahip Değilsin")
                                                end
                                            end, "lockpick")
                                        end
                                    end
...
function UTK:Plant()
    ...
    exports["np-taskbar"]:startUI(4500, self.text.thermal)
    Citizen.Wait(1500)
    ...
    Citizen.Wait(4000)
    exports["np-taskbar"]:startUI(12000, self.text.burning)
    DeleteObject(bag)
...
function UTK:Lockpick()
    ...
    exports["np-taskbar"]:startUI(6000, self.text.lockpick..": Aşama 1")
    Citizen.Wait(6500)
    exports['mythic_notify']:SendAlert("success", "Aşama 1 "..self.text.stage)
    SetEntityCoords(ped, loc.x, loc.y, loc.z, 1, 0, 0, 1)
    SetEntityHeading(ped, loc.h)
    TaskPlayAnim(ped, animDict, "a_uncuff", 8.0, 8.0, 6000, 1, 1, 0, 0, 0)
    exports["np-taskbar"]:startUI(6000, self.text.lockpick..": Aşama 2")
    Citizen.Wait(6500)
    exports['mythic_notify']:SendAlert("success", "Aşama 2 "..self.text.stage)
...
function UTK:Hack()
    ...
    Citizen.Wait(2000)
    Brute()
    exports["mythic_notify"]:SendAlert("success", "My Computer Aç Ve HackConnecte Giriş Yap")
...
function UTK:IdCard()
    ...
    exports["np-taskbar"]:startUI(2000, UTK.text.card)
    Citizen.Wait(1500)
    ...
    elseif self.currentid == 2 then
        ...
        UTK.disableinput = false
        exports["np-taskbar"]:startUI(4000, UTK.text.using)
        local count = 4
        repeat
            Citizen.Wait(1000)
            count = count - 1
...
function Search(location)
    ...
    exports["np-taskbar"]:startUI(15000, UTK.text.search)
    Citizen.Wait(15000)
...
function Process(ms, text) exports["np-taskbar"]:startUI(ms, text) Citizen.Wait(ms) end
        ...
        if HasScaleformMovieLoaded(scaleform) and UsingComputer then
            ...
            if GetScaleformMovieFunctionReturnBool(ClickReturn) then
                ...
                elseif Hacking and program == 84 then
                    ...
                    Hacking = false
                    Ipfinished = true
                    exports["mythic_notify"]:SendAlert("success", "BruteForce.exe Çalıştır")
...
AddEventHandler("utk_oh:vaulttimer", function(method)
    if method == 1 then
        exports['mythic_notify']:SendAlert("error", "Zehirli gaz girene kadar 90 saniyeniz var.")
        UTK.starttimer = true
        UTK.vaulttime = 90
        Citizen.CreateThread(function()
            while true do
...
AddEventHandler("utk_oh:gas_c", function()
    Citizen.CreateThread(function()
        UTK.begingas = true
        exports["mythic_notify"]:SendAlert("error", "Kasa kapısı 60 saniye içinde otomatik olarak kapanacaktır!")
        while true do
            Citizen.Wait(1)
            if UTK.begingas then
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if UTK.begingas then
                ...
                UTK.stage2break = true
                UTK.stage4break = true
                exports["mythic_notify"]:SendAlert("error", "KASA KAPI KAPANIŞI!")
...
AddEventHandler("utk_oh:policenotify", function(toggle)
    ...
    if player.job.name == "police" then
        if toggle == 1 then
            exports["mythic_notify"]:SendAlert("infrom", "Merkez Bankası Alarmları Çaldı!", 10000, {["background-color"] = "#CD472A", ["color"] = "#ffffff"})
