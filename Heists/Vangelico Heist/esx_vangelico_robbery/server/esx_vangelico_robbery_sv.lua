...
RegisterServerEvent('zentih-doorlock:vangelico')
AddEventHandler('zentih-doorlock:vangelico', function(data)
	if not doorstatus then
		TriggerClientEvent('zentih-doorlock:vangelico', -1, data)
		--TriggerClientEvent('zentih-vangelico:rob', source)
		Citizen.Wait(Config.SecBetwNextRob)
		TriggerClientEvent('zentih-doorlock:vangelico', -1, false)
	else
...
RegisterServerEvent('zentih-doorlock:bonus')
AddEventHandler('zentih-doorlock:bonus', function(data)
	if not bonuscracked then
		TriggerClientEvent('zentih-doorlock:bonus', -1, data)
		--TriggerClientEvent('zentih-vangelico:rob', source)
		Citizen.Wait(Config.SecBetwNextRob)
		TriggerClientEvent('zentih-doorlock:bonus', -1, false)
	else
		TriggerClientEvent('mythic_notify:client:SendAlert', source, {type='error', text='Bu yakın zamanda kullanıldı!'})
	end
...
RegisterServerEvent('zentih-vangelico:addBank')
AddEventHandler('zentih-vangelico:addBank', function(data)
	local xplayer = ESX.GetPlayerFromId(source)
	xplayer.addAccountMoney('bank', data)	
end)

RegisterServerEvent('zentih-doorlock:hacked')
AddEventHandler('zentih-doorlock:hacked', function(data)
	if not hacked then
		TriggerClientEvent('zentih-doorlock:hacked', -1, data)
		--TriggerClientEvent('zentih-vangelico:rob', source)
		Citizen.Wait(Config.SecBetwNextRob)
		TriggerClientEvent('zentih-doorlock:hacked', -1, false)
	else
		TriggerClientEvent('mythic_notify:client:SendAlert', source, {type='error', text='Bu yakın zamanda kullanıldı!'})
	end
...
ESX.RegisterServerCallback('zentih-vangelico:checkItem', function(source, cb, item)
	local xPlayer = ESX.GetPlayerFromId(source)
...
AddEventHandler('esx_vangelico_robbery:toofar', function(robb)
	...
	local xPlayers = ESX.GetPlayers()
	rob = false
	TriggerClientEvent('zentih:asked', source)
...
AddEventHandler('esx_vangelico_robbery:rob', function(robb)
	...
	if Stores[robb] then
		...
		if rob == false then
			...
			-- bonuscracked = false
			-- hacked = false
			TriggerClientEvent('mythic_notify:client:SendAlert', source, {type='error', text=_U('started_to_rob') .. store.nameofstore .. _U('do_not_move')})
			TriggerClientEvent('mythic_notify:client:SendAlert', source, {type='error', text=_U('alarm_triggered')})
			TriggerClientEvent('mythic_notify:client:SendAlert', source, {type='error', text=_U('hold_pos')})
...
RegisterServerEvent('zentih-hacknotify')
AddEventHandler('zentih-hacknotify', function()
	local source = source
	local xPlayer = ESX.GetPlayerFromId(source)
	local xPlayers = ESX.GetPlayers()
	local store = Stores[robb]
...
RegisterServerEvent('zentih:removekart')
AddEventHandler('zentih:removekart', function()

	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)

	xPlayer.removeInventoryItem('secure2', 1)
end)

RegisterServerEvent('zentih:removeusb')
AddEventHandler('zentih:removeusb', function()
